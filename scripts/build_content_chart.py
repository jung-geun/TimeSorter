...
d = json.loads(Path("presentation/01_model_comparison/content_n30.json").read_text())
b, ins = d["models"]["base"], d["models"]["instruct"]
...
# 모델별 (schema%, content%) — SFT/DPO는 유효 스키마 항상 출력 → schema=content=90
models = [
    ("Qwen3.5-4B\n(no FT)", pct(ins["schema_pass"]), pct(ins["content_pass"])),
    ("Base\n(no FT)",        pct(b["schema_pass"]),   pct(b["content_pass"])),
    ("+ SFT v4",             90.0, 90.0),
    ("+ DPO v5",             90.0, 90.0),
]
...
ax.set_title("포맷(스키마) vs 추론(내용) 분리 통과율 (Qwen3.5-4B, n=30)",
...
# no-FT 갭 강조
ax.annotate("0% → 43%\n추론은 했지만\n포맷만 틀림", xy=(0 + w/2, 43.3), xytext=(0.55, 70),
