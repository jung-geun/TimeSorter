#!/usr/bin/env python
"""v3: 사용자 확장 PPT(25슬라이드)의 실험 섹션만 최신화 (앞 데이터셋 섹션 완전 보존).

- 슬라이드 1~18: 손대지 않음 (사용자 확장 DATA SOURCE / 데이터셋 섹션)
- 슬라이드 19~21: 차트 이미지 교체(n=150) + 캡션 수정
- 슬라이드 23~24: KEY FINDING 표 수치 n=150 갱신
- 슬라이드 25(결론): 수치 갱신
...
SRC = "/tmp/user_deck_v3.pptx"
OUT = "/mnt/hdd/WD_8TB/code/TimeSorter/presentation/TimeSorter_발표자료_v3.pptx"
...
# ── 슬라이드 19: 학습 여정 (chart_milestones) ────────────────────────────────
replace_pic(slides[18], ASSETS / "chart_milestones.png")
set_textbox(slides[18], "데이터 큐레이션",
            [("성능 점프는 모델 구조가 아니라 데이터 큐레이션(+20.6%p)과 모델 업그레이드(+8.0%p)에서",
              dict(size=13, bold=True, color=NAVY))])

# ── 슬라이드 20: 4B 단계별 (chart_progression) ───────────────────────────────
replace_pic(slides[19], ASSETS / "chart_progression.png")
set_textbox(slides[19], "어댑터 없음",
            [("어댑터 없음 0% → Base 14.7% → SFT 85.3% → DPO 85.3% (held-out n=150)",
              dict(size=13, italic=True, color=GRAY))])

# ── 슬라이드 21: 시나리오별 (chart_scenario) ─────────────────────────────────
replace_pic(slides[20], ASSETS / "chart_scenario.png")
set_textbox(slides[20], "의존성 체인만",
            [("대부분 시나리오 90~100% · 의존성 체인만 47%로 공통 막힘 (SFT·DPO 동일, n=150)",
              dict(size=13, bold=True, color=NAVY))])

# ── 슬라이드 23: KEY FINDING ① 표 (n=30 → n=150) ─────────────────────────────
# 표: 행1 instruct 0.0%, 행2 base 16.7%, 행3 +SFT 90.0%
set_table_cell(slides[22], 0, 1, "통과율(n=150)")
set_table_cell(slides[22], 2, 1, "14.7%")   # base
set_table_cell(slides[22], 3, 1, "85.3%")   # +SFT
# 본문: 내용만 채점 시 instruct 34% 추가 설명
set_textbox(slides[22], "instruct 모델 0%",
...
# ── 슬라이드 24: KEY FINDING ② 표 (체인 67 → 47) ─────────────────────────────
set_table_cell(slides[23], 1, 1, "90~100%")  # 비체인 SFT
set_table_cell(slides[23], 2, 1, "47%")       # 체인 SFT
set_table_cell(slides[23], 2, 2, "47%")       # 체인 DPO

# ── 슬라이드 25: 결론 수치 갱신 ───────────────────────────────────────────────
set_textbox(slides[24], "최종 성과",
...
ids = list(sldIdLst)
concl = ids[24]            # 원래 25번째(결론) sldId
new_ids = ids[25:]         # 방금 추가된 3장
for nid in new_ids:
    sldIdLst.remove(nid)
