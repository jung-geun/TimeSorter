...
| 후처리 (dpo_v3) | 전 규칙 통과율 | 비고 |
|----------------|--------------|------|
| 없음 | 56.7% | past_rank 52건, chain 39건이 지배적 |
| 전면 rerank | 43.3% | **역효과** — 4축 점수가 시각·체인 순서 정보를 표현 못해 파괴 |
| **guard rerank** | **62.7%** | 지난 일정 시그니처만 강등, 모델 순서 보존 (현 기본) |

> 이 실측으로 `--rerank` 동작을 변경: 순서 일치율<50%일 때만 전면 재정렬, 평시엔 guard.
