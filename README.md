...
| **DPO v5** | **`outputs/dpo_q35_4b_v5`** | dpo_pairs_v5 (on-policy) | 1,368쌍 | **0.1706** | val **85.3%** (128/150, n=150; 초기 n=30=90.0%) · reward_acc=98.9% · margin=3.495 · 잔여 위반: 체인 47% |
...
| Qwen3.5-4B-Base | no adapter | 14.7% | 2% | 10% | 50% | 40% | 3% | 150 |
| Qwen3.5-4B | **SFT v4** | **85.3%** | 96% | 90% | 100% | 93% | 47% | 150 |
| Qwen3.5-4B | **DPO v5** | **85.3%** | 96% | 90% | 100% | 93% | 47% | 150 |
...
> - n=150: held-out 150샘플 전체 평가 (Qwen3-4B·Qwen3.5-4B 4-way·Qwen3.5-9B 최종) / n=30: 9B 4-way 빠른 평가
> - Qwen3.5-4B/9B no adapter: thinking mode 출력 → JSON 파싱 전부 실패 (0.0%)
> - Base no adapter: RLHF 없어 형식 불완전 (4B 14.7% / 9B 3.3%)
...
> **동일 표본(n=150) 비교**: 4B 85.3% vs 9B 88.7% (+3.4%p). 9B는 당일시각(+7%p)·체인(47→57%, +10%p)에서 앞서나 **의존성 체인은 4B·9B 모두 47~57%로 미해결**. 체인은 모델 크기가 아닌 **SFT 데이터 보강**이 필요한 문제임을 재확인. (리스크는 4B 100% > 9B 95%)

#### 학습 지표 비교 (4B vs 9B)

![학습 지표 4B vs 9B](assets/chart_train_metrics_4b_9b.png)

| 지표 | Qwen3.5-4B | Qwen3.5-9B |
|------|-----------|-----------|
| 파라미터 | 4.2B | 9.4B |
| GPU / VRAM | RTX 3080 Ti 12GB | RTX 4090 24GB |
| 정밀도 | 4-bit QLoRA NF4 | 4-bit QLoRA NF4 |
| **SFT** (동일 데이터 sft_v4_train 6,056) | | |
| · train_loss | 0.642 → **0.309** | → **0.320** |
| · token accuracy | **90.8%** | **91.5%** |
| · steps / epochs | 513 / 3 | 513 / 3 |
| · 학습 시간 | 9.5h | 11.5h |
| **DPO** (데이터 다름) | dpo_v5 on-policy 1,368 | dpo_v4_extra 1,978 |
| · train_loss | 0.166 | 0.65 |
| · reward accuracy | 97.5% | 88.0% |
| · reward margin | 3.50 | 0.099 |
| · 학습 시간 | ~0.5h | 2.5h |
| **검증 통과율 (n=150)** | 85.3% | 88.7% |

> **SFT는 동일 데이터라 공정 비교** — 9B의 SFT 성능은 4B와 거의 동일(token acc +0.7%p, loss·시간 유사). 모델을 2.3× 키워도 SFT 태스크 학습 자체엔 큰 이득 없음.
> **DPO는 데이터가 달라 직접 비교 불가** — 4B(reward margin 3.50)는 on-policy hard-negative라 분리가 크고, 9B(0.099)는 v4_extra라 작다. margin 차이는 모델이 아니라 **DPO 데이터 난이도** 때문.
...
| Qwen3-4B → Qwen3.5-4B | 77.3% → **85.3%** (+8.0%p, 둘 다 n=150) | 모델 업그레이드 효과 |
